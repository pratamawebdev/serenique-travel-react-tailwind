import Card from "../../Fragments/Admin/Dashboard/Card";
import AdminLayouts from "../Wrapper/AdminLayouts";
import useFetch from "../../../hooks/useFetch";
import { urlStaticProfileImage } from "../../../utils/data";
...
const DashboardLayouts = () => {
  ...
  } = useFetch("api/v1/user");
  const {
    data: dataUsers,
    loading: loadingUsers,
    error: errorUsers,
  } = useFetch("api/v1/all-user");
  const {
    data: dataBanners,
    loading: loadingBanners,
    error: errorBanners,
  } = useFetch("api/v1/banners");
  const {
    data: dataPromos,
    loading: loadingPromos,
    error: errorPromos,
  } = useFetch("api/v1/promos");
  const {
    data: dataCategories,
  ...
  const totalUser = dataUsers?.data?.length;
  const totalBanner = dataBanners?.data?.length;
  const totalPromo = dataPromos?.data?.length;
  const totalCategory = dataCategories?.data?.length;
  const totalActivity = dataActivities?.data?.length;
  const total =
    totalUser + totalBanner + totalPromo + totalCategory + totalActivity;

  const dataDashboard = [
    {
      title: "Total",
      count: total.toString(),
      icon: "M7.5 7.5h-.75A2.25 2.25 0 004.5 9.75v7.5a2.25 2.25 0 002.25 2.25h7.5a2.25 2.25 0 002.25-2.25v-7.5a2.25 2.25 0 00-2.25-2.25h-.75m-6 3.75l3 3m0 0l3-3m-3 3V1.5m6 9h.75a2.25 2.25 0 012.25 2.25v7.5a2.25 2.25 0 01-2.25 2.25h-7.5a2.25 2.25 0 01-2.25-2.25v-.75",
  ...
  const adminUsers = dataUsers?.data
        ...
        <>
          <div className="grid gap-5 mb-6 xl:grid-cols-2 2xl:grid-cols-3">
            {dataDashboard.map((v, i) => (
              <Card key={i} title={v.title} count={v.count}>
                <div
                  className={`inline-flex items-center justify-center flex-shrink-0 w-16 h-16 mr-6 text-black ${v.bgColor} rounded-full`}
                >
                  <svg
                    aria-hidden="true"
                    fill="none"
              ...
              <div className="flex items-center justify-center w-40 ">
                ...
                >
                  ...
                  <path
                    ...
                    fill="#0a7e71"
                  />
                  <g clipPath="url(#07e7d08c-c6f9-475f-8fa6-c69f2c72c8fe)">
                    <polygon
